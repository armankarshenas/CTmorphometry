%% This script has been written to analyse the information content of the bits within the 16-bit TIF files
function [] = Randomness(~,~)
clear 
clc
close all
path = input("Please enter the path to the top directory");
cd(path)
Sample_fish = input("Please enter the name of a directory for a sample scan");
cd(Sample_fish);

dirPattern = fullfile(pwd,'*.jpg');
all = dir(dirPattern);
for z=1 : length(all)
    if all(z).isdir ~= 1
        fprintf("File %s \n",all(z).name);
